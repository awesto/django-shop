...
class ProductSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, product, *args, **kwargs):
        ...
        if 'view' in kwargs['context'] and 'with_direct_siblings' in kwargs['context']['view'].kwargs and kwargs['context']['view'].kwargs['with_direct_siblings']:
    ...
    def serializer_direct_siblings(self, product, previous=None, next=None):
        if self.prev:
            previous = { "product_name":  self.prev.product_name,  "slug": self.prev.slug, "img_name": self.prev.images.first().original_filename, "img_url":  self.prev.images.first().url }
        if self.next:
            next = { "product_name": self.next.product_name,  "slug":  self.next.slug, "img_name": self.next.images.first().original_filename, "img_url": self.next.images.first().url }
        return previous , next

    def get_object_with_direct_siblings(self, product, request=None, *args,  **kwargs):
        if not request:
        ...
        if not hasattr(self, '_product_direct_siblings'):
            ...
            if hasattr(ProductModel, 'translations'):
                filter_siblings.update(translations__language_code=get_language_from_request(request))
