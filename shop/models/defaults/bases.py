...
class BaseOrderExtraInfo(models.Model):
    """
    A holder for extra textual information to attach to this order.
    """
    order = models.ForeignKey(get_model_string('Order'), related_name="extra_info",
            verbose_name=_('Order'))
...
class BaseExtraOrderPriceField(models.Model):
    """
    ...
    """
    order = models.ForeignKey(get_model_string('Order'), verbose_name=_('Order'))
...
class BaseExtraOrderItemPriceField(models.Model):
    """
    This will make Cart-provided extra price fields persistent since we want
    to "snapshot" their statuses at the time when the order was made
    """
    order_item = models.ForeignKey(get_model_string('OrderItem'), verbose_name=_('Order item'))
    label = models.CharField(max_length=255, verbose_name=_('Label'))
    value = CurrencyField(verbose_name=_('Amount'))
