...
class CatalogTest(ShopTestCase):
    ...
    def test_add2cart(self):
        ...
        # update quantity
        payload = dict(cart_item, quantity=3)
        response = self.client.put(payload['url'], data=json.dumps(payload), content_type='application/json')
        self.assertEqual(response.status_code, 200)
        cart_item = json.loads(response.content.decode('utf-8'))['cart_item']
        ...
        self.assertEqual(cart['subtotal'], "€ 11.97")
        self.assertEqual(cart['total'], "€ 11.97")
        ...
        cart = json.loads(response.content.decode('utf-8'))
        self.assertDictContainsSubset({
            'items': [], 'total_quantity': 0, 'num_items': 0, 'subtotal': "€ 0.00",
            'total': "€ 0.00"}, cart)
        ...
        self.assertDictContainsSubset({'product': sdhc_4gb.pk, 'quantity': 0, 'extra': {}}, watch_item)
        ...
        cart_item = cart['items'].pop()
        self.assertDictContainsSubset({"product": sdhc_4gb.pk, 'quantity': 1, 'extra': {}}, cart_item)
